...
class getUserDetailsAPI(Resource):
    ...
    #    uId = some_json['ID']
        ...
        if uId is None:
          f.close()
          return({'error':'Missing User ID in the Json.','success':''},409)
        ...
        if userDetails is None:
          f.close()
          return({'error':'User ID not found.','success':''},404)

        f.write("User Details: "+ userDetails.decode('utf-8'))
        f.close()
        result_json = { 'user_id': uId, 'user_details': userDetails.decode('utf-8')}
        return ({'error':'','success':result_json}, 200)
...
class verifyOtpAPI(Resource):
    ...
    def post(self):
      ...
      try:
        ...
        if l_otp is None:
            f.close()
            return({'error':'OTP expired. Get new OTP.','success':''},400)
        else:
...
" + str(l_otpFromUser))
        if l_otp != l_otpFromUser:
            f.close()
            return({'error':'Wrong OTP.','success':''},400)
        else:
            f.close()
...
api.add_resource(getUserDetailsAPI, '/getUserDetails/<userID>')
